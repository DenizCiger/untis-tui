use super::shared::{
    centered_message_lines, filter_chip, fit_text, line_with_right, render_input_text,
    styled_cell, to_single_line, truncate_text,
};
use super::theme::{
...
fn render_absence_history_pane(
    ...
    let block = Block::default()
        ...
        .title("History")
        .title(Line::from(format!("{selected_count}/{}", filtered.len())).alignment(Alignment::Right));
    ...
    let mut lines = vec![Line::from(vec![
        ...
        styled_cell(" ", Some(HEADER_BG), Some(DIM_GRAY)),
        styled_cell(
            &fit_text("State", status_chip_width),
            Some(HEADER_BG),
            Some(DIM_GRAY),
        ),
    ])];
    ...
    if state.main.absences.loading_initial {
        ...
        for (offset, absence) in visible.iter().enumerate() {
            ...
            lines.push(Line::from(vec![
                ...
                styled_cell(&fit_text(&note, note_col_width), row_bg, Some(Color::Indexed(15))),
                styled_cell(" ", row_bg, None),
                Span::styled(
                    format!(
                        " {:>width$} ",
